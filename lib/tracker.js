...
import {
  esc, slugify, uniqueKey, optBtn, wireInlineRename, wireOptionListActions, wireColorSwatches, positionPopover,
} from "./optionPopover";
...
export function mountTracker(refs, db) {
  ...
  function duplicateVideo(id){
    ...
    var ref = doc(collection(db, 'videos'));
    setDoc(ref, {
      title: v.title, contentType: v.contentType, taskType: v.taskType,
      priority: v.priority, status: v.status, deadline: v.deadline || '', publishDate: v.publishDate || '',
      links: (v.links||[]).map(function(l){ return { platform: l.platform, url: l.url }; }),
      assigneeId: v.assigneeId || '',
      createdAt: serverTimestamp()
    }).catch(function(e){ console.error('Failed to duplicate video', e); });
  }
  ...
  function rowHtml(v){
    ...
    return '<tr data-id="'+v.id+'" class="'+(over?'overdue':'')+'">'+
      ...
      '<td><div class="row-actions">'+
        '<button class="row-del" data-action="duplicate" title="Duplicate">'+
          '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>'+
        '</button>'+
        '<button class="row-del" data-action="delete" title="Delete">'+
  ...
  function openPopoverFor(cell, field, id){
    ...
    if(field==='contentType' || field==='taskType'){
      ...
      pop.innerHTML = '<div class="popover-title">Status</div>'+
        STATUSES.map(function(s){
          ...
          return optBtn({dataField:'status', value:s.key, selected:s.key===v.status, label:s.label, iconHtml:icon, editList:'status', editKey:s.key, removeList:'status', removable:STATUSES.length>1, colorList:'status', color:s.color});
        }).join('') +
    ...
    wireColorSwatches(pop, {
      onColorChange: function(listName, key, color){
        ...
        if(item){ item.color = color; persistConfig(); render(); openPopoverFor(cell, field, id); }
      }
    });
  ...
  function onRootClick(e){
    ...
    if(dupBtn){
      var trDup = dupBtn.closest('tr');
      duplicateVideo(trDup.getAttribute('data-id'));
      return;
    }
