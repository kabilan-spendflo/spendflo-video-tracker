...
export function mountTracker(refs, db) {
  ...
  var DEFAULT_PLATFORM_ICON = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>';
  ...
  var TRASH_SVG = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg>';
  ...
  function wireOptionListActions(pop){
    pop.querySelectorAll('.remove-opt').forEach(function(btn){
      btn.addEventListener('click', function(e){
        e.stopPropagation();
        removeOption(btn.getAttribute('data-remove-list'), btn.getAttribute('data-remove-key'));
        closePopover();
      });
    });
    pop.querySelectorAll('[data-add-option]').forEach(function(input){
      ...
      input.addEventListener('keydown', function(e){
        if(e.key==='Enter' && input.value.trim()){
          addOption(input.getAttribute('data-add-option'), input.value.trim());
          closePopover();
        }
      });
    });
